...
import { toastSucc, toastErr } from "services/ApiAll";
import { addShipment, getClients, deleteShipment, editShipment } from "services/ApiAdmin";
...
export const EditShip = ({ getData, formData, setFormData, setFocusShip, clients }) => {
...
const ShipmentList = ({ getShipments, role }) => {
                        ...
                        {shownShips.length > 0 ? (
                            shownShips.map((shipment, index) =>
                                <div className="bg-white shadow-lg rounded-xl overflow-hidden mb-8 flex flex-col">
                                    ...
                                    { getCard({ quote: shipment, index, to: (`/${role}/shipment/` + shipment.shipmentId), model: "shipment" }) }
